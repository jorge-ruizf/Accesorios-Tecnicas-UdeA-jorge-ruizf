...
public class ModelAccesoriosTable extends AbstractTableModel {
    ...
    public void RemoveAccesorio(int index){
        if (index >= 0 && index < this.listAccesorios.size()) {
            this.listAccesorios.remove(index);
            this.fireTableDataChanged();
        }
    }
    
    
    public void ModificarAccesorio(int index, String nombre, int cantidad, String material){
        if (index >= 0 && index < this.listAccesorios.size()) {
            this.listAccesorios.get(index).setNombre(nombre);
            this.listAccesorios.get(index).setCantidad(cantidad);
            this.listAccesorios.get(index).setMaterial(material);
            this.fireTableDataChanged();
        }
    }
